...
from mae_model_amgl import fusion_model_mae_2
...
def train_a_epoch(model,train_data,all_data,patient_and_time,patient_sur_type,batch_size,optimizer,epoch,format_of_coxloss,args):
    ...
    if len(args.train_use_type) != 1:
        ...
        if 'img' in fea_dict and fea_dict['img'] is not None:
            fea_vec = fea_dict['img']  # shape: [1, feature_dim]
            if fea_vec.dim() == 2 and fea_vec.shape[0] == 1:
                features_list.append(F.normalize(fea_vec, dim=1))
                contrastive_labels.append(torch.floor(torch.tensor(patient_and_time[id] / 12)).long())
...
def main(args): 
    ...
    if cancer_type == 'lihc':
        patients = joblib.load('../LIHC/lihc_patients.pkl')
        sur_and_time = joblib.load('../LIHC/lihc_sur_and_time.pkl')
        all_data = joblib.load('../LIHC/lihc_data.pkl')
        seed_fit_split = joblib.load('../LIHC/lihc_split.pkl')
    elif cancer_type == 'lusc': 
        patients = joblib.load('../LUSC/lusc_patients.pkl')
        sur_and_time = joblib.load('../LUSC/lusc_sur_and_time.pkl')
        all_data=joblib.load('../LUSC/lusc_data.pkl')
        seed_fit_split = joblib.load('../LUSC/lusc_split.pkl')

    elif cancer_type == 'luad': 
        patients = joblib.load('../LUAD/luad_patients.pkl')
        sur_and_time = joblib.load('../LUAD/luad_sur_and_time.pkl')
        all_data=joblib.load('../LUAD/luad_data.pkl')
        seed_fit_split = joblib.load('../LUAD/luad_split.pkl')
    elif cancer_type == 'ucec': 
        patients = joblib.load('../UCEC/ucec_patients.pkl')
        sur_and_time = joblib.load('../UCEC/ucec_sur_and_time.pkl')
        all_data=joblib.load('../UCEC/ucec_data.pkl')
        seed_fit_split = joblib.load('../UCEC/ucec_split.pkl')
    elif cancer_type == 'stad':
        patients = joblib.load('../STAD/stad_patients.pkl')
        sur_and_time = joblib.load('../STAD/stad_sur_and_time.pkl')
        all_data=joblib.load('../STAD/stad_data.pkl')
        seed_fit_split = joblib.load('../STAD/stad_split.pkl')
    elif cancer_type == 'esca':
        patients = joblib.load('../ESCA/esca_patients.pkl')
        sur_and_time = joblib.load('../ESCA/esca_sur_and_time_154.pkl')
        all_data = joblib.load('../ESCA/esca_data.pkl')
        seed_fit_split = joblib.load('../ESCA/esca_split.pkl')
        ...
        for train_index, test_index in kf.split(patients,kf_label):
            ...
            with torch.no_grad():
                for id in test_data:
                    data = all_data[id]
                    data.to(device)
                    (one_x,multi_x),fea,(att_1,att_2,att_each_2),_ = t_model(data,args.train_use_type,args.train_use_type,mix=args.mix)


                    gnn_time[id] = one_x.cpu().detach().numpy()[0]
                    fold_fusion_test_ci[id] = one_x.cpu().detach().numpy()[0]
                    print(data.sur_type.cpu().detach().numpy()[0],one_x.cpu().detach().numpy()[0],patient_and_time[id],id)
                    one_test_feature[id] = {}
                    for i,type_name in enumerate(['img','rna','cli']):
                        if type_name in data.data_type:
                            (one_,_),one_fea,(_,_,_),_ = t_model(data,args.train_use_type,use_type=[type_name],mix=args.mix)
                    ...
                    for i,two_type_name in enumerate([['img','rna'],['img','cli'],['rna','cli']]):
                        (one_,two_),one_fea,(_,_,_),_ = t_model(data,args.train_use_type,use_type=two_type_name,mix=args.mix)
            ...
            torch.save(t_model.state_dict(), '/homec/wanglanlan/wll_code/HGCN-main/STAD/save_model/model/'+sys_time.strftime('%Y-%m-%d')+'_'+str(seed)+'_'+str(n_fold)+'.pkl')
    ...
    joblib.dump(all_gnn_time,'/homec/wanglanlan/wll_code/HGCN-main/STAD/save_model/'+sys_time.strftime('%Y-%m-%d-%H-%M')+'all_gnn_time'+'.pkl')
    joblib.dump(all_each_model_time,'/homec/wanglanlan/wll_code/HGCN-main/STAD/save_model/'+sys_time.strftime('%Y-%m-%d-%H-%M')+'all_each_model_time'+'.pkl')
...
def get_params():
    ...
    parser.add_argument("--start_seed", type=int, default=1238, help="start_seed")
    parser.add_argument("--repeat_num", type=int, default=100, help="Number of repetitions of the experiment")
    parser.add_argument("--fusion_model", type=str, default="fusion_model_mae_2", help="")
    # parser.add_argument("--fusion_model", type=str, default="fFusionModelMGCM", help="")
    parser.add_argument("--drop_out_ratio", type=float, default=0.5, help="Drop_out_ratio")
    parser.add_argument("--lr", type=float, default=0.00006, help="Learning rate of model training")
    parser.add_argument("--epochs", type=int, default=60, help="Cycle times of model training")

    parser.add_argument("--batch_size", type=int, default=16, help="Data volume of model training once")
    ...
    parser.add_argument("--adjust_lr_ratio", type=float, default=0.2, help="adjust_lr_ratio")
